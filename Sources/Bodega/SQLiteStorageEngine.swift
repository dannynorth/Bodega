...
public actor SQLiteStorageEngine<Key: StorageKey>: StorageEngine where Key.KeyType: SQLite.Value & Hashable & Sendable, Key.KeyType.Datatype: Equatable {
    ...
    public init?(directory: FileManager.Directory, databaseFilename filename: String = "data") {
        ...
        do {
            ...
            let keyRow = self.keyRow

            try self.connection.run(self.storageTable.create(ifNotExists: true) { table in
                table.column(keyRow, primaryKey: true)
                table.column(Self.expressions.dataRow)
                table.column(Self.expressions.createdAtRow, defaultValue: Date()) // this is wrong; should be CURRENT_TIMESTAMP
                table.column(Self.expressions.updatedAtRow, defaultValue: Date()) // this is wrong; should be CURRENT_TIMESTAMP
            })
        } catch {
            return nil
        }
    ...
    /// Writes `Data` to the database with an associated ``Key``.
    /// - Parameters:
    ...
    public func write(_ data: Data, key: Key) throws {
        ...
        // Normally this should be an INSERT OR REPLACE call,
        // but SQLite.swift doesn't appear to support CURRENT_TIMESTAMP as a default value
        // Therefore we must manually check to see if it exists and
        // manually provide the createdAt value
        ...
        if try self.keyExists(key) {
            ...
            values.append(Self.expressions.createdAtRow <- Date())
            try self.connection.run(
    ...
    /// Writes an array of `Data` items to the database with their associated ``Key`` from the tuple.
    /// - Parameters:
    ///   - dataAndKeys: An array of the `[(Key, Data)]` to store
    ///   multiple `Data` items with their associated keys at once.
    public func write(_ keysAndValues: [(key: Key, value: Data)]) throws {
    ...
    /// - Parameters:
    ///   - key: A ``Key`` for matching `Data`.
    /// - Returns: The `Data` stored if it exists, nil if there is no `Data` stored for the `Key`.
    ...
    /// Reads `Data` items based on the associated array of ``Key``s provided as a parameter.
    /// - Parameters:
    ///   - keys: A `[Key]` for matching multiple `Data` items.
    /// - Returns: An array of `[Data]` stored on disk if the `Key`s exist,
    ...
    /// - Parameters:
    ///   - keys: A `[Key]` for matching multiple `Data` items.
    /// - Returns: An array of `[(Key, Data)]` if the `Key`s exist,
    ...
    public func readAllValues() throws -> [Data] {
        let query = self.storageTable.select(Self.expressions.dataRow)
    ...
    /// - Returns: An array of the `[Data]` and it's associated ``Key``s.
    public func readAllKeysAndValues() throws -> [(key: Key, value: Data)] {
        let query = self.storageTable.select(self.keyRow, Self.expressions.dataRow)
    ...
    /// Removes `Data` items from the database based on the associated array of ``Key``s provided as a parameter.
    /// - Parameters:
    ///   - keys: A `[Key]` for matching multiple `Data` items to remove.
    public func remove(keys: [Key]) throws {
    ...
    /// Returns the modification date for the `Data` item matching the ``Key``, if it exists.
    /// - Parameters:
    ///   - key: A ``Key`` for matching `Data`.
    /// - Returns: The modification date of the `Data` on disk if it exists, nil if there is no `Data` stored for the ``Key``.
