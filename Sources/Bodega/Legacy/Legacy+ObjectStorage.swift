...
extension Legacy {
    ...
    public actor ObjectStorage<Object: Codable> {
        private let storage: any StorageEngine<CacheKey, Data>
        ...
        public init(storage: any StorageEngine<CacheKey, Data>) {
        ...
        public func objects(forKeys keys: [CacheKey]) async -> [Object] {
            let dataItems = (try? await storage.readKeysAndValues(keys: keys)) ?? []
            
            do {
                return try dataItems.map({ try self.decoder.decode(Object.self, from: $0.value) })
            } catch {
                return []
            }
        }
        ...
        public func keyCount() async -> Int {
            return (try? await storage.keyCount()) ?? 0
        }
