...
extension Legacy {
    ...
    public actor DiskStorageEngine: StorageEngine {
        ...
        public func write(_ keysAndValues: [(key: Legacy.CacheKey, value: Data)]) async throws {
            for (key, value) in keysAndValues {
                try self.write(value, key: key)
            }
        }
        ...
        public func readKeysAndValues(keys: [Legacy.CacheKey]) async throws -> [(key: Legacy.CacheKey, value: Data)] {
            var dataAndKeys: [(key: CacheKey, value: Data)] = []
        ...
        public func readDataAndKeys(keys: [CacheKey]) async -> [(key: CacheKey, data: Data)] {
            return try! await self.readKeysAndValues(keys: keys).map { ($0, $1) }
        }
        ...
        public func readAllData() async -> [Data] {
            return try! await self.readAllValues()
        }
        ...
        public func readAllValues() async throws -> [Data] {
            ...
            var values: [Data] = []
            for key in keys {
                if let value = self.read(key: key) {
                    values.append(value)
                }
            }
            return values
        ...
        public func readAllKeysAndValues() async throws -> [(key: Legacy.CacheKey, value: Data)] {
        ...
        public func readAllDataAndKeys() async -> [(key: CacheKey, data: Data)] {
            return try! await self.readAllKeysAndValues().map { ($0, $1) }
        }
        ...
        public func remove(keys: [Legacy.CacheKey]) async throws {
            for key in keys {
        ...
        public func keysExist(_ keys: [Legacy.CacheKey]) async throws -> [Legacy.CacheKey] {
            let allKeys = self.allKeys()
...
fileprivate extension Legacy.DiskStorageEngine {
    ...
    func concatenatedPath(key: Legacy.CacheKey) -> URL {
